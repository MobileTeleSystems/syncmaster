...
class ServerSettings(BaseModel):
    ...
    debug: bool = Field(
        ...
        description=textwrap.dedent(
            """
            [Enable debug output in responses][server-configuration-debug].
    ...
    cors: CORSSettings = Field(
        ...
        description="[CORS settings][server-configuration-cors]",
    )
    monitoring: MonitoringSettings = Field(
        default_factory=MonitoringSettings,
        description="[Monitoring settings][server-configuration-monitoring]",
    )
    openapi: OpenAPISettings = Field(
        default_factory=OpenAPISettings,
        description="[OpenAPI.json settings][server-configuration-openapi]",
    )
    static_files: StaticFilesSettings = Field(
        default_factory=StaticFilesSettings,
        description="[Static files settings][server-configuration-static-files]",
    )
