...
Брокер сообщений - это компонент, используемый [REST API Сервером][server]/[Планировщиком][scheduler] для связи с [Рабочим процессом(воркером)][worker].

SyncMaster может работать практически с любым брокером, поддерживаемым [Celery](https://docs.celeryq.dev).
Но единственный брокер, который мы тестировали, это [RabbitMQ](https://www.rabbitmq.com/).
...
Пожалуйста, следуйте [инструкции по установке RabbitMQ](https://www.rabbitmq.com/docs/download).
