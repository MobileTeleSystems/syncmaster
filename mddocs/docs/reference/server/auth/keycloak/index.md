...
```mermaid
sequenceDiagram
participant Client as Client (User from Browser)
...
This provider ensures integration with OAuth2 Gateway and maintains the standard authorization flow as described in the Keycloak Auth Provider section. It also uses the [python-keycloak](https://pypi.org/project/python-keycloak/) library for interactions with the Keycloak server and handles the token exchange process similarly.

**Configuration**

`OAuth2GatewayProvider` uses almost the same configuration models as `KeycloakAuthProvider` — namely:

::: syncmaster.server.settings.auth.oauth2_gateway.OAuth2GatewayProviderSettings

::: syncmaster.server.settings.auth.oauth2_gateway.OAuth2GatewayKeycloakSettings
